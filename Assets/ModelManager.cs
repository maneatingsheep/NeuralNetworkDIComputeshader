...
public class ModelManager : MonoBehaviour
{
    ...
    private Model BreedModels(Model parent1, Model parent2) {
        ...
        for (int w = 0; w < parent1.Weights.Length; w++) {
            int xSize = parent1.Weights[w].Weights.GetLength(0);
            int ySize = parent1.Weights[w].Weights.GetLength(1);
            for (int i = 0; i < xSize; i++) {
                for (int j = 0; j < ySize; j++) {
                    float p1Val = parent1.Weights[w].Weights[i, j];
                    float p2Val = parent1.Weights[w].Weights[i, j];
                    //select one random
                    ...
                    resModel.Weights[w].Weights[i, j] = selection;
                }
            }
        }
    }
